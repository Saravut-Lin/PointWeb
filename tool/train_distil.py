...
def get_parser():
    ...
    parser.add_argument('--distill-alpha', type=float, default=0.5,
                        help='Weight between CE loss and distillation loss')
    parser.add_argument('--distill-temp', type=float, default=4.0,
                        help='Temperature for softening distributions in distillation')
    args = parser.parse_args()
    assert args.config is not None
    cfg = config.load_cfg_from_cfg_file(args.config)
    if args.opts is not None:
        cfg = config.merge_cfg_from_list(cfg, args.opts)
    # Merge distillation args into config
    cfg.distill = args.distill
    cfg.teacher_path = args.teacher_path
    cfg.distill_alpha = args.distill_alpha
    cfg.distill_temp = args.distill_temp
    return cfg
...
def main():
    ...
    # Knowledge distillation: load teacher model if requested
    teacher_model = None
    criterion_kd = None
    if args.distill:
        assert args.teacher_path, "Please specify --teacher-path for distillation"
        logger.info(f"Loading teacher model from {args.teacher_path}")
        teacher_net = Model(c=args.fea_dim, k=args.classes, use_xyz=args.use_xyz)
        ckpt = torch.load(args.teacher_path, map_location='cuda')
        teacher_net.load_state_dict(ckpt['state_dict'])
        teacher_net.eval()
        teacher_model = torch.nn.DataParallel(teacher_net.cuda())
        for p in teacher_model.parameters():
            p.requires_grad = False
        criterion_kd = nn.KLDivLoss(reduction='batchmean').cuda()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train, mIoU_train, mAcc_train, allAcc_train = train(
            ...
            teacher_model, criterion_kd, args.distill_alpha, args.distill_temp)
...
def train(train_loader, model, criterion_ce, optimizer, epoch,
          teacher_model=None, criterion_kd=None, alpha=0.5, temp=4.0):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if teacher_model is not None:
            ...
            T = temp
            log_p = F.log_softmax(output / T, dim=1)
            q = F.softmax(teacher_output / T, dim=1)
            loss_kd = criterion_kd(log_p, q) * (T * T)
...
'''
sh tool/train_distil.sh market pointweb
'''