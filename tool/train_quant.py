...
def get_parser():
    ...
    parser.add_argument('--quant-method', type=str, choices=['qat','ptq'], default=None,
                        help='Quantization method: "qat" for quantization-aware training, "ptq" for post-training quantization')
    parser.add_argument('--quant-backend', type=str, default='fbgemm',
                        help='Quantization backend for QAT or PTQ')
    parser.add_argument('--calib-loader', type=str, choices=['train','val'], default='val',
                        help='Data loader to use for calibration during PTQ')
    args = parser.parse_args()
    assert args.config is not None
    cfg = config.load_cfg_from_cfg_file(args.config)
    if args.opts is not None:
        cfg = config.merge_cfg_from_list(cfg, args.opts)
    # Merge quantization CLI args into config
    cfg.quant_method = args.quant_method
    cfg.quant_backend = args.quant_backend
    cfg.calib_loader = args.calib_loader
    return cfg
...
def main():
    ...
    # Quantization setup if requested
    if args.quant_method == 'qat':
        logger.info(f"Preparing model for QAT (backend={args.quant_backend})")
        model.module.fuse_model()
        model.module.prepare_qat(backend=args.quant_backend)
    ...
    # Finalize quantized model if requested
    if args.quant_method == 'qat':
        logger.info("Converting QAT-trained model to quantized version")
        model.module.convert_quantized()
        torch.save(model.module.state_dict(), os.path.join(args.save_path, 'quantized_qat_model.pth'))
    elif args.quant_method == 'ptq':
        logger.info(f"Applying post-training quantization (backend={args.quant_backend}) using {args.calib_loader} loader")
        model.module.fuse_model()
        calib_loader = train_loader if args.calib_loader == 'train' else val_loader
        model.module.apply_post_training_quantization(calib_loader, backend=args.quant_backend)
        torch.save(model.module.state_dict(), os.path.join(args.save_path, 'quantized_ptq_model.pth'))
    writer.close()
...
'''
sh tool/train_quant.sh market pointweb --quant-method ptq --quant-backend fbgemm --calib-loader train
'''